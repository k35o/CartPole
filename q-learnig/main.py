...
class Qlearning:
    ...
    def update_Qtable(self, observation, action, reward, observation_next):
        ...
        td = reward + GAMMA * max(self.q_table[state_next]) - self.q_table[state, action]
...
class Env():
    def __init__(self, env, Qlearning_class):
        self.env = env
        self.Qlearning_class = Qlearning_class
    ...
    def run(self):
        ...
        Qlearning = self.Qlearning_class(num_states, num_actions)
        ...
        for episode in range(NUM_EPISODES):
            ...
            for step in range(MAX_STEPS):
                ...
                # 行動を求める
                action = Qlearning.decide_action(observation, episode)
                ...
                if done:  # ステップ数が200経過するか、一定角度以上傾くとdoneはtrueになる
                    ...
                    reward = 0
                # Qテーブル, Vを更新する
                Qlearning.update_Qtable(observation, action, reward, observation_next)
                # 観測値を更新する
